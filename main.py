...
def main():
  ...
  #Obliczenie ilości segmentów o konkretnym stopniu dopasowania. Dopasowania zostały podzielone na grupy o następujących przedziałach procentowych: match1 (0-74%), match2 (75-84%), match3 (85-94%) i match4 (95-100%).
  ...
  match4_2 = json_data["analyseLanguageParts"][0]["data"]["match100"]["segments"]["mt"] #Z dopasowań o wartości 100% bierzemy pod uwagę jedynie te pochodzące z tłumaczenia maszynowego oraz non-translatables, ponieważ wychodzimy z założenia, że dopasowania z pamięci tłumaczeniowych są na tyle dobrej jakości, że nie wymagają postedycji.
  ...
  #Obliczanie wartości dopasowań poprzez pomnożenie ilości segmentów o konkretnym stopniu dopasowania razy przypisaną danej grupie wartość
  ...
  if (mtqe >= 0.7):
    #Obliczenie ilości stron rozliczeniowych (1800 zzs) do postedycji poprzez odjęcie powtórzeń, dopasowań 101% oraz dopasowań 100% z pamięci tłumaczeniowej od całkowitej liczby stron w danym tekście
    ...
    #Obliczenie procentowej ilości tłumaczenia maszynowego. Jako tłumaczenie maszynowe zaliczono również non-translatables.
    ...
    translators_rate = float(input("Stawka tłumacza: ")) #Stawka tłumacza za tłumaczenie 1 strony rozliczeniowej tekstu
    translation_rate = pages_for_postediting * translators_rate #Obliczenie stawki jaką dostałby tłumacz za tłumaczenie
    verification_rate = translation_rate * 0.3 #Przyjmuje się, że stawka za weryfikację tłumaczenia stanowi 30% stawki za tłumaczenie
    postediting_rate = verification_rate + (1 - mtqe) * verification_rate #W skład stawki za postedycję wchodzi weryfikacja tekstu oraz jakość tłumaczenia maszynwego i dopasowań z pamięci tłumaczeniowych. Im lepsza jakość tłumaczenia maszynowego tym niższa stawka za postedycję i vice versa.
    extra_postediting_rate = postediting_rate * 1.1 #Dodatkowe 10% od Supertłumacza
    if (extra_postediting_rate < 5): #Stawkę poniżej 5 zł zaokrąglamy do 5 zł
      print("Stawka za postedycję:", 5, "zł")
    elif (extra_postediting_rate < 10 and extra_postediting_rate > 5): #Stawkę pomiędzy 5-10 zł zaokrąglamy do 10 zł
